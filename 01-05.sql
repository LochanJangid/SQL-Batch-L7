...
-- find total sales, avg sales for north region but for mobile, laptop
select sum(amount) as total_sales, avg(amount) as avg_sales from sales where region = 'north' and product in ('mobile', 'laptop');

-- u need to find out the maximum amount and the min. amount for the ferbuary month
...
-- find the total trasactions for each product but the amit should not be the sales person
select product, sum(amount) from sales where sales_person != 'amit' group by product;

-- find the sales done by each person for the amount of greater than 10000
select sales_person, sum(amount) from sales where amount > 10000 group by sales_person;

-- find the number of trasactions done from day1 to day7 for any month
select count(*) from sales where day(sale_date) between 1 and 7;

-- find the total amount, avg amout, max. amount for each product of each region
select region, product, sum(amount), avg(amount), max(amount) from sales group by region, product;

-- find the total trasaction amount for each product where product should be sold more than 13 times
select product, sum(amount), count(amount) from sales group by product having count(amount) >= 13; 